# Configurable Notification System — PulseNotify

A production-ready, highly extensible Configurable Notification System built with **NestJS (Node.js)** on the backend and **Angular (v18, Standalone Components, Reactive Forms)** on the frontend, backed by **SQLite** via **Prisma ORM**.
...
| | |
|---|---|
| **Live Demo** | [notification-sys.vercel.app](https://notification-sys.vercel.app/) |
| **Video Walkthrough** | [Watch on Loom](https://www.loom.com/share/954a070e01aa434a88b0d0cd500030cc) |

---

## Screenshots

### Rules Engine — Home Page
View, search, filter, and toggle all configured notification rules at a glance.

![Rules Engine](./docs/screenshots/rules-engine.png)

### New Rule Builder
Compose rules with a dynamic conditions builder (AND logic), multi-channel dispatch, and `{{field}}` template interpolation.

![New Rule Builder](./docs/screenshots/rule-builder.png)

### Dispatch Logs
Paginated, filterable audit trail of every notification dispatched — including status (`SENT` / `FAILED` / `SKIPPED`) and full event details.

![Dispatch Logs](./docs/screenshots/dispatch-logs.png)

---

## UI Design — White Theme with Orange Accents and Glassmorphism

The system features a custom light-mode visual design system:

1. **Light Ambient Glassmorphism**: Soft off-white canvas with dynamic multi-point ambient orange gradients (`radial-gradient` mesh lighting). Glassmorphic panels with backdrop blur (`backdrop-filter: blur(16px)`), crisp white semi-transparent borders, and soft warm drop-shadows.
2. **Vibrant Orange Accent System**: Primary buttons and active states feature a warm orange (`#ff5500`, `#ff7d29`), with glowing ambient button shadows (`box-shadow: 0 8px 24px -4px rgba(255, 85, 0, 0.35)`) and glowing orange input focus rings.
3. **Floating Glass Navigation Bar**: Sticky floating pill-shaped navbar containing the **PulseNotify** logo and pill-style navigation tabs with active tab shadows.
